import type { Metadata } from "next";
import { Plus_Jakarta_Sans, JetBrains_Mono } from "next/font/google";
import "../globals/styles/globals.css";
import { ThemeProvider } from "./provider/theme-provider";




/** 
 * font-extralight	200
 * font-light	300
 * font-normal	400
 * font-medium	500
 * font-semibold	600
 * font-bold	700
 * font-extrabold	800
**/
const PlusJakartaSansFont = Plus_Jakarta_Sans({
    variable: "--font-plus-jakarta-sans",
    weight: ["200", "300", "400", "500", "600", "700", "800"],
    subsets: ["latin"],
    style: ["normal", "italic"],
})

const JetBrainsMonoFont = JetBrains_Mono({
    variable: "--font-jet-brains-mono",
    weight: ["200", "300", "400", "500", "600", "700", "800"],
})


export const metadata: Metadata = {
    title: "Dedi Suharman — Fullstack Developer",
    description: "Portfolio Dedi Suharman, Fullstack Developer yang berpengalaman membangun aplikasi web scalable dengan Rust, Node.js, Next.js, dan infrastruktur self-hosted",
};

export default function RootLayout({ children }: LayoutProps<"/">) {
    return (
        <html
            lang="id"
            className={`${PlusJakartaSansFont.variable} ${JetBrainsMonoFont.variable} h-full antialiased scroll-smooth`}
            suppressHydrationWarning
        >
            <head>
                <meta charSet="UTF-8" />
                <meta name="viewport" content="width=device-width, initial-scale=1.0" />
            </head>
            <body id="home" className="transition-colors duration-300">
                <ThemeProvider>
                    {children}
                </ThemeProvider>
            </body>

        </html>
    );
}
